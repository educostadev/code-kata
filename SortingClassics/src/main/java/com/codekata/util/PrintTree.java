...
public class PrintTree {
        ...
        while (nn != 0) {
            ...
            for (Node n : level) {
                if (n == null) {
                    ...
                    next.add((Node) n.left);
                    next.add((Node) n.right);
    ...
    public static void horizontal(Node root) {
        if (root.right != null) {
            printTree((Node) root.right, true, "");
        }
        printNodeValue(root);
        if (root.left != null) {
            printTree((Node) root.left, false, "");
        }
    }
    ...
    private static void printTree(Node node, boolean isRight, String indent) {
        if (node.right != null) {
            printTree((Node) node.right, true, indent + (isRight ? "        " : " │      "));
        }
        ...
        printNodeValue(node);
        if (node.left != null) {
            printTree((Node) node.left, false, indent + (isRight ? " │      " : "        "));
        }
    }
